...
import { ChevronDown, ChevronUp, ThumbsUp, AlertTriangle, Lightbulb, RotateCcw } from "lucide-react";
import { Button } from "@/components/ui/button";
import AnimatedScoreBar from "../simulation/AnimatedScoreBar";
...
export default function ScenarioEvaluationCard({ evaluation, onRestart }: Props) {
  ...
  return (
    ...
    >
      <div className="rounded-xl border border-border bg-card p-5 space-y-3">
        <p className="text-xs text-muted-foreground uppercase tracking-wide">Résultat — Scénario critique</p>
        <p className="text-sm font-medium text-foreground">{evaluation.verdict}</p>
        <AnimatedScoreBar score={evaluation.note} />
      </div>

      <div className="rounded-xl border border-border bg-card p-5 space-y-3">
        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Critères</p>
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        ...
        >
          <span className="flex items-center gap-2 font-medium text-foreground">
            ...
            Voir la gestion idéale
          </span>
          {showIdeal ? <ChevronUp size={14} className="text-muted-foreground" /> : <ChevronDown size={14} className="text-muted-foreground" />}
        </button>
        {showIdeal && (
